...
class JSONCreation():
    ...
    def generate_monthly_list(self, df, metric_id, origin_key, start_date = None, metric_type='default'):
        ...
        df_tmp['unix'] = (df_tmp['date'].dt.to_period("M").dt.start_time).astype(np.int64)      
    ...
    def create_landingpage_json(self, df):
        ...
        for metric_id in ['throughput', 'txcount', 'stables_mcap', 'rent_paid', 'market_cap', 'fees']:
    ...
    def create_export_project_labels_parquet(self):
        exec_string = """
            SELECT 
        ...
        upload_parquet_to_cf_s3(self.s3_bucket, f'{self.api_version}/oli/project_labels', df, self.cf_distribution_id)
        print(f'DONE -- OLI project_labels.parquet export')
    ...
    def create_fundamentals_json(self, df):
        ...
        ## filter date to be in the last 90 days
        df = df.loc[df.date >= (datetime.now() - timedelta(days=90)).strftime('%Y-%m-%d')]
